...
const Profile = () => {
  ...
  const renderProgressTracker = (req, isIncoming) => {
    ...
    return (
      <div className="mt-4 p-4.5 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-850 flex flex-col gap-3.5 w-full">
        ...
        <div className="w-full h-2.5 bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden relative border border-slate-200/40 dark:border-slate-800">
          <div 
        ...
        <div className="grid grid-cols-3 text-center text-[10px] font-bold text-slate-450 select-none">
          <div className="text-emerald-500 dark:text-emerald-400">1. Approved</div>
          <div className={isDelivered ? 'text-emerald-500 dark:text-emerald-400 font-extrabold' : ''}>2. Handed Over (Cook)</div>
          <div className={isReceived ? 'text-emerald-500 dark:text-emerald-400 font-extrabold' : ''}>3. Received (Buyer)</div>
        </div>

        <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-slate-100 dark:border-slate-900 mt-1 justify-between sm:items-center">
          {isIncoming ? (
            <>
            ...
            <>
              <div className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold flex flex-col gap-0.5">
                <span>Cook: {isDelivered ? '✅ Handed Over' : '⌛ Waiting for Cook to Hand Over'}</span>
                <span>Buyer: {isReceived ? '✅ Confirmed Received' : '❌ Needs Receipt Confirmation'}</span>
              </div>
              {!isReceived && (
                <button
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          ...
          <div className="glass-panel p-6 rounded-3xl glow-card border border-slate-100 dark:border-slate-850">
            ...
            <form onSubmit={handleProfileUpdate} className="flex flex-col gap-4">
              <div>
                <label className="block text-xs font-extrabold text-slate-750 dark:text-slate-300 uppercase mb-2">Display Name</label>
                <input
                  required
                  type="text"
                  value={name}
              ...
              <div>
                <label className="block text-xs font-extrabold text-slate-750 dark:text-slate-300 uppercase mb-2 flex items-center gap-1.5">
                        ...
                        {req.status === 'pending' && (
                          <div className="flex gap-2">
                            <button
                              ...
                              className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1 active:scale-95 transition-all shadow-md shadow-emerald-500/10 cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="bg-slate-100 hover:bg-rose-50 dark:bg-slate-800 dark:hover:bg-rose-950/20 text-slate-600 hover:text-rose-500 font-bold px-3 py-1.5 rounded-xl text-xs transition-colors cursor-pointer"
                            >
                          ...
                          <div className="flex items-center">
                            {ratedPosts[req.foodPostId._id] ? (
                              ...
                              <button
                                ...
                                className="bg-amber-500 hover:bg-amber-600 text-white font-extrabold px-4 py-2 rounded-xl text-xs flex items-center gap-1 active:scale-95 transition-all shadow-md shadow-amber-500/10 cursor-pointer"
                              >
                ...
                <div className="flex flex-col gap-3">
                  {outgoingReqs.map((req) => (
                    <div key={req._id} className="p-5 rounded-3xl border border-slate-150 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-900/30 flex flex-col gap-4 w-full">
                      ...
                      {renderProgressTracker(req, false)}
                    </div>
                  ))}
                </div>
              )}
